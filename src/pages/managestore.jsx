...
import { Flowbite, Table, Button, Checkbox, Label, TextInput, Alert } from 'flowbite-react';
...
function ManageStore() {
  ...
  function edit(store_name1, store_business_url1) {
  ...
  function getStatus() {
    if (status === lang("en", "created")) {
      ...
      return (
        <Alert className="mt-8 -mb-8" theme={customTheme1} color="failure" icon={HiOutlineXCircle} onDismiss={() => dispatch(resetError())} rounded>
            ...
            <div className="px-8 py-6 dark:bg-gray-800">
              <div className="flex flex-row gap-4 items-end">
                <div className="block pb-1">
                  <Label htmlFor="name1" value="店舗名" />
                </div>
                <TextInput type="name" placeholder="" value={store_name} onChange={e => setStoreName(e.target.value)} required />
                <div className="block pb-1">
                  <Label htmlFor="name2" value="店舗Googleビジネスurl" />
                </div>
                <TextInput type="name" className="w-80" value={store_business_url} onChange={e => setStoreBusinessURL(e.target.value)} placeholder="" required />
                <div className="block pb-1">
                  <Label htmlFor="name3" value="調査調査url" />
                </div>
                <TextInput type="name" className="w-80" placeholder="" value={(store_name !== "" && store_business_url !== "") ? `${config.server_url}/questionnaire?name=${store_name}` : ""} required readOnly />
                <Button type="button" onClick={() => { dispatch(createOrUpdate({ store_name, store_business_url })) }}>登録 / 変更</Button>
              </div>
            </div>
                    ...
                    : store.map(item => (
                      <Table.Row key={item.id} className="bg-white dark:border-gray-700 dark:bg-gray-800">
                        ...
                        <Table.Cell className="flex flex-row">
                          <a href="#" onClick={() => edit(item.store_name, item.store_business_url)} className="font-medium text-cyan-600 hover:underline dark:text-cyan-500">
                            Edit
                          </a>
